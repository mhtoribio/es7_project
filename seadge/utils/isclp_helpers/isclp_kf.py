...
def build_psi_w_delta(N_STFT_half: int, M: int, L: int, psi_wLP: float, psi_wSC: np.ndarray, alpha_ISCLP_KF: float) -> tuple[list, list]:
    ...
    for k in range(N_STFT_half):
        ...
        Psi_w_tilde_init.append(np.diag(diag_entries)) # between (52) and (53) 
        Psi_w_delta.append((1-alpha_ISCLP_KF) * Psi_w_tilde_init[k])

    return Psi_w_delta, Psi_w_tilde_init
